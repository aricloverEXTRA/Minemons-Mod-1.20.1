...
public class CardRenderer {
    ...
    public static void drawCard(DrawContext ctx, TextRenderer tr,
        ...
        if (!drawCardArt(ctx, card, cardId, artX, artY2, artW, artH2)) {
            Identifier tex = getTextureForCard(card, cardId);
            try {
    ...
    private static boolean drawCardArt(DrawContext ctx, Card card, String cardId, int x, int y, int w, int h) {
        ...
        if (card instanceof MinemonCard && CardAssetManager.isEntityArt(cardId) && client.world != null) {
            EntityType<?> type = getEntityTypeForCard(card, cardId);
            if (type != null) {
    ...
    private static Identifier getTextureForCard(Card card, String cardId) {
        String texturePath = card.getTexturePath();
        if (texturePath != null && !texturePath.isEmpty()) {
            String normalized = texturePath.replace("\\", "/");
            if (!normalized.startsWith("textures/")) {
                if (normalized.startsWith("entity/") || normalized.startsWith("item/") || normalized.startsWith("items/")) {
                    normalized = "textures/" + normalized;
                } else if (card instanceof MinemonCard) {
                    normalized = "textures/entity/" + normalized;
                } else {
                    normalized = "textures/item/" + normalized;
                }
            }
            if (!normalized.endsWith(".png")) normalized = normalized + ".png";
            normalized = normalized.replace("textures/items/", "textures/item/");
            return new Identifier("minemons", normalized);
        }
        return CardAssetManager.getTexture(cardId);
    }

    private static EntityType<?> getEntityTypeForCard(Card card, String cardId) {
        EntityType<?> configuredType = CardAssetManager.getEntityType(cardId);
        if (configuredType != null) return configuredType;

        String texturePath = card.getTexturePath();
        if (texturePath == null || texturePath.isEmpty()) return null;
        String normalized = texturePath.replace("\\", "/");
        if (normalized.startsWith("entity/")) normalized = normalized.substring("entity/".length());
        if (normalized.endsWith(".png")) normalized = normalized.substring(0, normalized.length() - 4);

        String candidate = normalized.contains("/") ? normalized.substring(normalized.lastIndexOf('/') + 1) : normalized;
        EntityType<?> exact = Registries.ENTITY_TYPE.getOrEmpty(new Identifier("minecraft", candidate)).orElse(null);
    ...
    private static void renderEntityArt(DrawContext ctx, Entity entity, int x, int y, int w, int h) {
        ...
        float entityHeight = entity instanceof LivingEntity living ? Math.max(1.0f, living.getHeight()) : 1.8f;
        float scale = Math.min(w * 0.72f, h * 0.78f) / entityHeight;
        float rotation = (client.world == null ? 0 : (client.world.getTime() % 360) * 4f) + client.getTickDelta();

        if (entity instanceof LivingEntity living) {
            living.bodyYaw = rotation;
            living.prevBodyYaw = rotation;
            living.headYaw = rotation;
            living.prevHeadYaw = rotation;
        }

        matrices.translate(x + w / 2f, y + h * 0.88f, 100f);
        matrices.scale(scale, scale, scale);
        matrices.multiply(new Quaternionf().rotateXYZ((float)Math.toRadians(-12), (float)Math.toRadians(180), 0));

        EntityRenderDispatcher dispatcher = client.getEntityRenderDispatcher();
        dispatcher.setRenderShadows(false);
        dispatcher.render(entity, 0, 0, 0, rotation, 1f, matrices, consumers, dispatcher.getLight(entity, 1f));
        consumers.draw();
