...
public class CardDefinition {
    ...
    private final int trainerValue;
    ...
    public CardDefinition(JsonObject json) {
        ...
        this.duration = json.has("duration") ? json.get("duration").getAsInt() : 0;

        // Trainer properties
        this.trainerEffect = json.has("trainerEffect") ? json.get("trainerEffect").getAsString() : json.has("effect") ? json.get("effect").getAsString() : "";
        this.trainerValue = json.has("value") ? json.get("value").getAsInt() : 0;

        // Place properties
        this.placeEffect = json.has("placeEffect") ? json.get("placeEffect").getAsString() : json.has("effect") ? json.get("effect").getAsString() : "";
        this.affectedElement = json.has("affectedElement") && !json.get("affectedElement").isJsonNull() ? json.get("affectedElement").getAsString() : null;
        this.placeValue = json.has("value") ? json.get("value").getAsInt() : 0;
    }
    ...
    public int getTrainerValue() { return trainerValue; }
    public String getPlaceEffect() { return placeEffect; }
    public Element getAffectedElement() { return affectedElement == null ? null : Element.valueOf(affectedElement.toUpperCase()); }
    public int getPlaceValue() { return placeValue; }
    ...
    public Card toCard() {
        return switch(type) {
            ...
            case CONSUMABLE -> new ConsumableCard(
                id, displayName, getElement(), getRarity(),
                ConsumableCard.ConsumableEffect.valueOf(consumableEffect.toUpperCase()),
                consumableValue, duration, description, texturePath
            );
            case TRAINER -> new TrainerCard(
                id, displayName, getRarity(),
                TrainerCard.TrainerEffect.valueOf(trainerEffect.toUpperCase()),
                trainerValue, description, texturePath
            );
            case PLACE -> new PlaceCard(
                id, displayName, getElement(), getRarity(),
                PlaceCard.PlaceEffect.valueOf(placeEffect.toUpperCase()),
                getAffectedElement(), placeValue, description, texturePath
            );
        };
    }
}
